...
from ..exceptions import BorgExecutionException, InvalidParameterException, SnapborgBaseException, SnapperExecutionException
...
def run_snapborg():
    ...
    if args.mode == "prune":
        ...
        backup(configs, recreate=args.recreate,
...
def list_snapshots(configs: List[SnapborgSnapperConfig]):
    ...
    for config in configs:
        ...
        for s in snapshots:
            print(f"  Snapshot {s.number} from {s.date} is backed up to the following borg repositories:")
            [
                print(f"    {repo.name} ({repo.path}): " + ("Yes" if s.is_backed_up(repo.name) else "No"))
                for repo in config.repos
            ]


def backup(snapper_configs: List[SnapborgSnapperConfig], recreate, prune_old_backups, dryrun, absolute_paths):
    """
...
def backup_config(config: SnapborgSnapperConfig, recreate, dryrun, absolute_paths) -> CommandResult:
    ...
    return CommandResult.from_children(repo_results, f"Backup of snapper config {config.name}" + (" (recreating archives)" if recreate else ""))

def backup_to_repo(repo: BorgRepo, snapper_config: SnapperConfig,
                   recreate: bool, dryrun: bool, mount_path: Optional[str] = None) -> CommandResult:
    ...
    try:
        ...
        snapshots = snapper_config.get_snapshots()
        if len(snapshots) == 0:
            return CommandResult.warn(f"No snapshots found for snapper config {snapper_config.name}!")
    except SnapperExecutionException as e:
        return CommandResult.err(f"Failed to get snapshots for snapper config {snapper_config.name}: {e}")

    retention_config = asdict(repo.retention_config)
    candidates = [
        snapshot
        for snapshot in get_retained_snapshots(
            snapshots, lambda s: s.date,
            **retention_config) if(not snapshot.is_backed_up(repo.name) or recreate)
    ]
    if len(candidates) == 0:
    ...
    try:
        with snapper_config.prevent_cleanup(snapshots=candidates, dryrun=dryrun):
            snapshot_results = [ 
                backup_snapshot(snapper_config, repo, candidate, recreate, dryrun=dryrun, mount_path=mount_path)
                for candidate in candidates
            ]
    except SnapperExecutionException as e:
        return CommandResult.err(f"Snapper error during handling of cleanup algorithm: {e}", children=snapshot_results)
    ...
    if any(it.status == ResultStatus.ERR for it in snapshot_results):
        ...
        if fail_after == False:
            return CommandResult.warn(
                f"Backup to optional repo {repo_string}",
                children=snapshot_results
            )
        elif fail_after == True:
            return CommandResult.err(
                f"Backup to mandatory repo {repo_string}",
                children=snapshot_results
            )
        else:
            # TODO handle error
            ...
            if len(snapshots) > 0 and len(backed_up) == 0:
                return CommandResult.err("No snapshots have been transferred to the borg repo yet!",
                    children=snapshot_results)
            newest_backed_up = max(
                backed_up,
                key=lambda x: x.date)
            if newest_backed_up.date + fail_after < datetime.now():
                return CommandResult.err(f"Backup to (time limited) optional repo {repo_string}:"
                ...
                return CommandResult.warn(f"Backup to (time limited) optional repo {repo_string}:"
...
def backup_snapshot(snapper_config: SnapperConfig, borg_repo: BorgRepo, snapshot: SnapperSnapshot,
                     recreate: bool, dryrun=False, mount_path=None) -> CommandResult:
    """Backup one specific snapper snapshot to one specific borg repository, potentially recreating
       an existing archive"""
    print(f"Backing up snapshot number {snapshot.number}, created at "
    ...
    try:
        ...
        borg_repo.backup(backup_name, snapshot.path, timestamp=snapshot.date_utc,
                         mount_path=mount_path)
        ...
        if e.returncode == BORG_RETURNCODE_ARCHIVE_EXISTS and not borg_repo.repo_config.is_old_config \
        ...
        return CommandResult.err(f"Borg reported an error backing up snapshot number {snapshot.number}: {e}")
    except SnapperExecutionException as e:
        return CommandResult.err(f"Borg reported an error backing up snapshot number {snapshot.number}: {e}")
